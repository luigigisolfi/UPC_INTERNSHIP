...
% 2) computes the vector field values along the orbit (HFEM_RTBP, HFEM_ETBP, HFEM_HILL3b)
...
% "closest" to HFEM. Circles: ETBP, Crosses: RTBP, Squares: HILL
...
%NOTE:
%the other example (which is basically very similar, named
%etbp_rtbp_residual_comparison_lissajous) showcases a very important
%feature that might be implemented to make this code much faster:
%Indeed, in that specific example, we compute the vector field
%HFEM_etbp_residual. This is basically HFEM - HFEM_etbp. 
%In other words, instead of computing HFEM(t,x) - HFEM_etbp(t,x) (as it is
%done in this example), in the other example we show that we can compute
%directly: HFEM_etbp_residual(t,x) thus reducing the computational time. 
...
for i = 1:1:length(x_orb(:,1))
        ...
        f2_val_full = HFEM_etbp(t_peri_peri, x);
        f2_val = f2_val_full(1:6);
        f3_val = HFEM_rtbp(t_rtbp, x);
        f4_val = hill3b(t_rtbp, x);

        nu_dot = f2_val_full(7); %HFEM etbp also gives the nu value. Here, we compute them to compare them
